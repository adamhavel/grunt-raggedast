...
module.exports = function(grunt) {
   ...
   grunt.registerMultiTask('raggedast', 'Adjust the text rag of your documents for better readability.', function() {
      ...
      var openingQuotes = '["\'´`„“‚‘‛‟‹«]';
      var closingQuotes = '["\'´`”’‛‟›»]';
      var gap = '(<[^>]+>|\\s|' + options.space + ')';
      var boundary = '(\\s|^|\\(|\\[|>|' + options.space + ')';
      var outsideTag = '(?![^<>]+>)';
      ...
      function process(contents) {
         ...
         if (options.extra && options.extra.length > 0) {
            ...
            contents = contents.replace(regexExtra, function(content) {
               return content.substr(0, 1) + content.substr(1).replace(regexWhitespace, options.space);
            });
         }
         ...
         if (options.limit > 0) {
            ...
            var spaceMatches = contents.match(regexSpace);

            // No reason to continue if the limit is way too high.
            if (spaceMatches && options.limit < spaceMatches.length) {
               ...
               var regexLimit = new RegExp(
                  '(([^\\s]|<[^>]+>)+?' + options.space + ')' /* 1. */
                  + '{' + (options.limit) + ',' + spaceMatches.length + '}', /* 2. */
               'gi');
      ...
      this.files.forEach(function(f) {
         ...
         if (options.type === 'json') {
            input = JSON.parse(src);

            async.each(Object.keys(input), function(key, callback) {
               if (options.ignore && new RegExp(options.ignore).test(key)) {
                  callback();
               } else {
                  var string = input[key];
                  input[key] = process(string);
                  callback();
               }
            }, function(err) {
            ...
            input(options.selector).each(function() {
               if (!options.ignore || !$(this).is(options.ignore)) {
                  var string = $(this).html();
                  $(this).html(process(string));
               }
            });
